import type { PickListItem } from '@/lib/api/merchant-orders';
import { pickLocale } from '@/lib/utils';

const esc = (s: string): string =>
  String(s ?? '').replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' })[c] ?? c);

/** Печать листа отбора (walk-sheet для сборщика) — отдельное окно + window.print, без зависимостей. */
export function printPickList(label: string, items: PickListItem[]): void {
  const rows = items
    .map((it, i) => {
      const cells =
        it.suggested
      ...
      const name = it.name ? pickLocale(it.name) : (it.sku ?? '');
