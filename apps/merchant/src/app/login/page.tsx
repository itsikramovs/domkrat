  ...
  async function onSubmit(values: FormData) {
    ...
    try {
      ...
      if (!roles.includes('MERCHANT') && !roles.includes('MERCHANT_STAFF')) {
        toast.error('Это вход только для мерчантов');
        setSubmitting(false);
        return;
      }
      setTokens(data);
  ...
  return (
    <div className="container py-12 min-h-[calc(100vh-100px)] flex items-center justify-center">
      <div className="w-full max-w-md">
        <Card>
          <CardHeader>
            <CardTitle>Вход для мерчантов</CardTitle>
            <CardDescription>
              Используйте email и пароль аккаунта с ролью MERCHANT.{' '}
              Демо: <span className="font-mono">merchant1@example.com / Test1234!</span>
            </CardDescription>
          </CardHeader>
          <CardContent>
