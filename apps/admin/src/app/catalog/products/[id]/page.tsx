...
import { ArrowLeft, Boxes, PackageCheck, Star, Trash2, Upload } from 'lucide-react';
...
function ProductImages({ product }: { product: AdminProductDetail }) {
  ...
  async function onFiles(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        await add.mutateAsync({ url, isPrimary: images.length === 0 });
      }
    ...
    <Card>
      <CardContent className="space-y-3 p-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-white">Фотографии</h2>
          <Button
            size="sm"
            variant="outline"
            disabled={busy}
        ...
        {images.length === 0 ? (
          <div className="rounded-lg border border-dashed border-white/15 p-6 text-sm text-white/55">
