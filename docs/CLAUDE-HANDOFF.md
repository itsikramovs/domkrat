...
> Дата последнего обновления: **2026-05-29** (вечер — цикл «маркетплейс + редизайн админки + склад»).
...
**Что добавлено в этом цикле сессий (всё задеплоено + запушено на github `master`):**
...
**Метрики:** 83 unit зелёные. Все 4 приложения собраны и перезапущены (`systemctl --user restart domkrat-*`). Admin: ~22 страницы, все на реальных endpoint'ах, 0 упавших API (проверено Playwright). `master` синхронизирован с `origin/master`.
