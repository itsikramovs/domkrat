...
6. **Маркетплейс — довести до конца (данные/бэкенд готовы, см. §0):**
   ...
   - **Кабинет мерчанта** (осталось): UI «мои предложения» (цена/остаток/статус) — нужны merchant-эндпоинты offer (`/merchant/offers/...` с проверкой владельца; сейчас offer CRUD только admin). Форма создания товара мерчантом создаёт через legacy-проекцию.
...
### Если нужно **довести маркетплейс до конца** (осталось: мерчант/поиск/seed, 1-2 дня):

- ✅ Витрина (`/p/[slug]` buy-box) и админ-панель управления товарами — СДЕЛАНЫ (см. §0).
- **Кабинет мерчанта** (`apps/merchant/src/app/products`): UI «мои предложения» (цена/остаток/статус). Нужны merchant-эндпоинты offer (сейчас offer CRUD только в `AdminProductsController` — добавить `MerchantOffersController` `/merchant/offers/...` с проверкой `offer.merchantId === currentMerchant`). Образец UI — `apps/admin/src/components/offers-manager.tsx`.
