...
**🆕 Маркетплейс-модель (2026-05-30, задеплоено; см. §0 первым):** товар больше НЕ плоский. Теперь 3 уровня: **карточка (контент) → варианты (простой список) → предложения продавцов (offer = продавец+цена+статус)**. Остаток/корзина/заказы/приёмка/FIFO ведутся по `offerId`. Мультипродавец: одна карточка → много предложений. Публичный API **обратно-совместим** (проецирует «основное предложение» на legacy `price`/`merchant`/`sku`), поэтому витрина и кабинет мерчанта работают **без изменений**. 104 товара мигрированы в карточки+варианты+предложения. Управление вариантами/предложениями — в админке (`/catalog/products/[id]`).
...
**Метрики:** 89 unit зелёные (было 83 + 6 на offer/variant). API+admin пересобраны и перезапущены; web/merchant НЕ трогались (обратная совместимость). 7 миграций Prisma (4 в этом цикле). `master` НЕ синхронизирован с `origin` — нужен `git push`.

**Блокеры боевого запуска (не делалось):** реальные платежи (Click/Payme/Uzum — 501 stubs), реальный SMS (Eskiz), legal. **Опционально:** числовые фасеты как range-слайдеры; вкладка приёмки в кабинете мерчанта (сейчас полный цикл у мерчанта есть, многострочный быстрый приход — в админке); фото в форме создания (сейчас — на странице редактирования после создания DRAFT).
...
- **Остаётся (следующая итерация):** витрина — селектор варианта + buy-box (список продавцов/«другие предложения») на `/p/[slug]`; кабинет мерчанта — UI «свои предложения/цена/остаток»; поиск — фасеты/релевантность по offer; алерты остатков пока агрегируются по (product,merchant), не по offer (приемлемо). ⚠️ `git push` — коммиты не запушены.
