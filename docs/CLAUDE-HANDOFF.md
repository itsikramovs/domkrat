...
✅ **Реальные изображения товаров (2026-05-30, задеплоено + запушено `50971af`).** Было 2/107 товаров с фото → стало **107/107**. Новый идемпотентный импортёр `apps/api/prisma/seed-product-images.ts`: по каждому товару без картинки строит англоязычный запрос из slug (тип + бренд, размеры/артикулы выкинуты — «brake pads brembo», «engine oil castrol magnatec»), тянет реальное CC-фото из **Openverse** (агрегатор Flickr/Wikimedia, без API-ключа), грузит в MinIO `product/<id>/`, создаёт primary `ProductImage` с прод-URL `https://cdn.domcrat.uz:443/...`. Дросселирование под лимиты Openverse (20/мин, 200/день), резюмируемо, валидация загрузок (image/\*, 5КБ–8МБ). Запуск: `pnpm --filter @domkrat/api exec tsx prisma/seed-product-images.ts`. **Проверено live:** все 107 отдаются `200 image/jpeg` через CDN, видны в `/products` и (после `POST /search/rebuild`) в поиске с `imageUrl`. ⚠️ Картинки CC — demo-grade; для боевого запуска заменить на лицензионные/каталожные фото с атрибуцией.
...
- ✅ **`seed.ts` обновлён под offer-модель** (`5025d22`) — `db:seed` работает на чистой БД (проверено scratch-прогоном, идемпотентно). `prod-seed.ts` товаров не сидит. Реальные фото товаров — `prisma/seed-product-images.ts` (`50971af`). `prisma/backfill-offers.ts` — исторический one-time скрипт фазы B (уже отработал), теперь не компилируется (читает удалённые колонки), в build/CI не входит.
