...
**🆕 Маркетплейс-модель (2026-05-30, задеплоено + запушено `33d3e48`; см. §0 первым):** товар больше НЕ плоский. Теперь 3 уровня: **карточка (контент) → варианты (простой список ярлыков) → предложения продавцов (offer = продавец+цена+статус)**. Остаток/корзина/заказы/приёмка/FIFO ведутся по `offerId`. Мультипродавец: одна карточка → много предложений. Публичный API **обратно-совместим** (проецирует «основное предложение» на legacy `price`/`merchant`/`sku`), поэтому витрина и кабинет мерчанта работают **без изменений и не пересобирались**. 104 товара мигрированы в карточки+варианты+предложения. Управление вариантами/предложениями + **полноценный 2-шаговый мастер добавления** (фото/варианты/предложения/приход) — в админке (`/catalog/products/[id]` и `/catalog/products/new`).
...
**Блокеры боевого запуска (не делалось):** реальные платежи (Click/Payme/Uzum — 501 stubs), реальный SMS (Eskiz), legal. **Маркетплейс — следующая итерация (данные/бэкенд готовы):** витрина — селектор варианта + buy-box (список продавцов) на `/p/[slug]`; кабинет мерчанта — UI «свои предложения/цена/остаток»; поиск — фасеты/релевантность по offer. **Опционально:** числовые фасеты как range-слайдеры; алерты остатков пока агрегируются по (product,merchant), не по offer.
...
- **~480 .ts/.tsx файлов**
- **132 теста** (89 unit + 43 E2E) — unit все зелёные
...
- ✅ Products: список, создание, редактирование с ImagesManager + CompatibilityManager (работает через обратно-совместимый API; UI «своих предложений» по offer — следующая итерация)
- ✅ Orders: confirm → ready → ship flow (FIFO-списание по offer)
- ⚠️ **Не пересобирался в маркетплейс-цикле** — продолжает работать на legacy-проекции API.
...
- **Каталог**: **Товары** (`/catalog/products` — список «от X · N предлож. · N продав.», 2-шаговый мастер добавления `/new`, детальная `/[id]` с `VariantsManager` + `OffersManager` (мультипродавец, inline-цена, приёмка по offer) + модерация), Категории (CRUD-дерево), Бренды (CRUD), **Характеристики** (`/attributes`)
...
- **Unit (89)**: PricingService, AuthService, PasswordService, OrdersService, PromoCodesService, AttributesService, Admin\* сервисы, BannersService, receipts (6), **ProductOffersService (6: projectPrimaryOffer / pickPrimaryOffer)** — все зелёные.
...
6. **Маркетплейс — довести до конца (данные/бэкенд готовы, см. §0):**
   - **Витрина `/p/[slug]`**: селектор варианта + buy-box (карточка уже отдаёт `variants[]` и `offers[]` аддитивно — нужно только нарисовать выбор варианта и список продавцов; add-to-cart переключить на `{offerId}`).
   - **Кабинет мерчанта**: UI «мои предложения» (цена/остаток/статус) поверх offer-эндпоинтов; форма создания товара мерчантом (сейчас создаёт через legacy-проекцию).
   - **Поиск**: индексация/фасеты по offer (сейчас Meili берёт цену основного предложения).
   - **E2E**: обновить под offer/variant (сейчас могут опираться на удалённые поля Product).
   - ⚠️ **seed.ts/prod-seed.ts** создают товары по СТАРОЙ модели (price/sku/merchantId на Product) → при пересоздании БД упадут. Переписать на card+variant+offer перед следующим `db:seed`.
...
### Если нужно **довести маркетплейс до конца** (витрина/мерчант/поиск, 2-3 дня):

- **Витрина `/p/[slug]`** (`apps/web/src/app/p/[slug]/page.tsx`): карточка УЖЕ отдаёт `variants[]` и `offers[]` аддитивно. Нарисовать селектор варианта + buy-box (список продавцов с ценой/«в наличии»); `AddToCartButton` → отправлять `{offerId}` вместо `{productId}` (бэкенд принимает оба). Цена «от X» в листингах уже корректна.
- **Кабинет мерчанта** (`apps/merchant/src/app/products`): UI «мои предложения» (цена/остаток/статус) поверх offer-эндпоинтов. Нужны merchant-эндпоинты offer (сейчас offer CRUD только admin — добавить `/merchant/offers/...` с проверкой владельца).
- **Поиск**: уже индексирует цену основного предложения; при желании — фасеты по offer.
- **Перед `db:seed`**: переписать `apps/api/prisma/seed.ts` (и `prod-seed.ts`) на создание card+variant+offer (сейчас пишут price/sku/merchantId на Product → упадут). Образец логики — `prisma/backfill-offers.ts` + `ProductsService.create`.
- E2E под новую модель.
...
- ⚠️ **`seed.ts`/`prod-seed.ts` НЕ обновлены** под новую модель (пишут price/sku/merchantId на Product) → упадут при `db:seed`. Миграция уже прошла, существующие 104 товара мигрированы (`prisma/backfill-offers.ts`). Для пересоздания БД сперва переписать сиды.
