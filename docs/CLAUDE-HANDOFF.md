...
**Блокеры боевого запуска (не делалось):** реальные платежи (Click/Payme/Uzum — 501 stubs), реальный SMS (Eskiz), legal. **Маркетплейс — ✅ доведён** (схема+бэкенд+миграция, админка, витрина buy-box, кабинет мерчанта, `seed.ts`). **Опционально осталось:** ценовые/числовые фасеты как range-слайдеры; алерты остатков пока агрегируются по (product,merchant), не по offer; обновить 43 e2e под offer/variant.
...
6. **Маркетплейс — довести до конца (данные/бэкенд готовы, см. §0):**
   ...
   - **E2E** (опц.): обновить под offer/variant (43 e2e могут опираться на удалённые поля Product).
...
- **E2E** (опц.): 43 e2e обновить под offer/variant (могут опираться на удалённые поля Product).
