...
**Опционально на будущее (не делал):** фасетные фильтры по характеристикам на витрине каталога (бэкенд `Attribute.isFilterable` готов), детальная модерация карточек товара админом, дальнейший визуальный полиш кабинетов.
