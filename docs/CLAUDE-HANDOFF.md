...
**Осталось по WMS (фаза 3):** cron-алерты залежавшегося товара (oldest_received_at >30/60/90д) и low-stock в `InventoryAlert`, QR-сканирование при размещении, аренда ячеек (monthlyRentalFee/RentalFee), трансферы между ячейками (StockMovement TRANSFER), admin-обзор приёмок всех мерчантов, привязка приёмки к продукту-контенту (шаг 5 §3.1).
