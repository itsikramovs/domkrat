...
✅ **WMS фаза 4: сборка/отгрузка из ячеек + инвентаризация (2026-05-31, задеплоено + запушено `1fdfd54`).** Запрос пользователя: «маркетплейс уровня Ozon/Я.Маркет + складской учёт», выбран первый блок = WMS фаза 4, фулфилмент **гибрид FBO+FBS**. **Слой 1 — сборка из ячеек (`0e9a1f5`):** новый `PickingService` (orders module) заменил неявный авто-FIFO-при-отгрузке на явный лист отбора. Семантика теперь **PROCESSING → pick (списать из выбранных ячеек) → ASSEMBLED → ship (только статус)**. `getPickList` (FIFO-подсказка ячеек + shortfall), `pick` (атомарный декремент ячейки с гардом gte + SHIPMENT-движения + снятие резерва агрегата + item PICKED/pickedFromCellId → ASSEMBLED когда все собраны), `autoAssemble` (быстрый FIFO), `ship`. Мерчант (FBS, `/merchant/orders/:id/pick-list|pick|ready|ship`) + админ (FBO, `AdminFulfillmentController` `/admin/orders/suborders/:id/...`). UI «Сборка из ячеек» на детали заказа в обоих кабинетах. **Слой 2 — инвентаризация (`1fdfd54`):** аддитивная миграция `wms_stock_count` (StockCount/StockCountItem/StockCountStatus, самодостаточные — без связей в существующие таблицы). `StockCountService`: create (снимок ожидаемых per-cell) → saveCounts → complete (расхождение → ADJUSTMENT_PLUS/MINUS + правка ячейки и агрегата + `alerts.scan`). Экран «Инвентаризация» (admin сайдбар «Склад», merchant «Ревизия»). **Проверено live:** FBS+FBO confirm→pick→ship (реальный декремент ячейки 5→3), SHIPMENT-движения, резервы сняты; ревизия create→save(недостача)→complete (ячейка 3→2, агрегат −1, ADJUSTMENT_MINUS). **105 unit + 47 e2e зелёные**, 5 сервисов active. **Дальше по приоритетам пользователя:** массовые операции/Excel-импорт, кабинет продавца (Ozon Seller), QR/этикетки/печать листа отбора, FBO supply-поставки.

🟡 **Изображения товаров — ОТКРЫТЫЙ ВОПРОС, источник не выбран (2026-05-31, `0ee5fdf`).** Пытались подтянуть реальные фото авто-поиском по бесплатным CC-стокам (**Openverse** = Flickr/Wikimedia, без ключа) — `prisma/seed-product-images.ts` строил запрос из slug и заливал в MinIO. **Не годится:** полный аудит (contact sheet всех 67 типов на 107 товаров) показал **~50% нерелевантных** даже с курируемыми по-типовыми запросами — слово «car» тянет фото целых машин, а у нишевых деталей (генератор, лямбда-зонд, коврики, сальники) нет нормальных CC-фото (получили дом, кота, классические авто). **Решение пользователя — убрать неверные фото:** удалены 105 авто-картинок (строки + объекты MinIO, по сигнатуре fileSize+altText; 2 «родных» изображения не тронуты), импортёр удалён, Meili переиндексирован. Витрина снова показывает чистые плейсхолдеры (иконка-гаечный-ключ), 0 битых картинок. **Что дальше (на выбор пользователя):** (1) **Pixabay** — бесплатный API-ключ (создаётся за минуту), фото чище и продуктовее CC → ожидаемо ~80–90% релевантно; (2) **пользователь даёт свои файлы** (папка по slug/SKU) — максимум точности, брендовые фото; (3) ручная кураторская подборка из Wikimedia Commons (медленно). Рекомендация — (1) или (2). Тип-классификатор (slug→~70 типов с англ-запросами) был написан и переиспользуем для любого источника — восстановить из этой сессии. ⚠️ Попутно в каталоге тестовый мусор: `test-ozon-…`, `batch-test-1/2` + старое `product/test.jpg` — почистить отдельно.
...
- **~480 .ts/.tsx файлов**
- **152 теста** (105 unit + 47 E2E) — все зелёные
...
- **Unit (105)**: PricingService, AuthService, PasswordService, OrdersService, PromoCodesService, AttributesService, Admin\* сервисы, BannersService, receipts (6), **ProductOffersService (6)**, **AlertsService (4)**, **PickingService (8: FIFO/мультиячейка/гард/идемпотентность/ship-без-списания)**, **StockCountService (4: расхождение→ADJUSTMENT/идемпотентность/ownership)** — все зелёные.
